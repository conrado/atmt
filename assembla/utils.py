...
def parse_datetime(string):
    stringdate = re.match(r"\d+-\d+-[T\d]+:\d+:\d+", string).group(0)
    date = datetime.strptime(stringdate, "%Y-%m-%dT%H:%M:%S")
    return date

def parse_date(string):
    stringdate = re.match(r"\d+-\d+-\d+", string).group(0)
    date = datetime.strptime(stringdate, "%Y-%m-%d").date()
    return date

def parse_file(string):
    fileid = re.match(r"\[\[(file|image):([\w\-_]+)\]\]", string).groups()[1]
    return fileid
